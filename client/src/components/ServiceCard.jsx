...
const ServiceCard = ({ service }) => {
  ...
  return (
    <div className="group relative flex flex-col overflow-hidden rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-2xl transition-all duration-500 ease-out hover:-translate-y-2">
      ...
      <div className="relative h-48 overflow-hidden">
        ...
        <div className="absolute top-3 right-3 z-20 flex items-center gap-1 rounded-full bg-white/90 px-2 py-1 text-xs font-bold text-slate-900 backdrop-blur-md shadow-sm opacity-0 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300">
          ...
          {service.rating || "New"} ({service.reviews || 0})
        </div>
        ...
        <div className="mt-auto pt-4 flex items-center justify-between border-t border-slate-100 dark:border-slate-700">
          <div className="flex flex-col">
            ...
            <div className="flex items-baseline">
              ...
              {/* Added Unit display here */}
              <span className="text-xs text-slate-500 dark:text-slate-400 ml-1">/ {service.unit || 'hr'}</span>
            </div>
          </div>
